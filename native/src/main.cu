...
int main(int argc, char** argv)
{
	...
	{
		tree[i].distribution = new float[3];
	}

	try
	{
		Timer t;
		generate_tree(d.attributes_compacted.data(), d.attributes.size(), d.classes.data(), d.attributes[0].size(), 3, n_levels, tree.data());
