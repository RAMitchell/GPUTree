...
void infogainSplit(Data&data, int n_levels){
	...
	thrust::device_vector<float > attributes = data.attributes_compacted;
	thrust::device_vector<bool> classes(n );
	...
	for (int level = 0; level < n_levels; level++){
		...
		cub::DeviceSegmentedReduce::Reduce(d_temp_storage, temp_storage_bytes, raw(indices), raw(node_best_infogain_index), n_nodes, raw(node_offsets), raw(node_offsets) + 1, reduce_op, 0);
